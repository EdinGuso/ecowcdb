...
def __all_forests(
    net: Network, min_edges: int, verbose: bool
    ...
    if verbose:
        with tqdm(
            total=num_cuts, desc="Selecting all valid forests from all cuts", unit="cut"
        ) as pbar:
            loop()
    else:
        loop()
...
def __subset_forests(
    net: Network, min_edges: int, num_forests: int, verbose: bool
) -> List[List[Tuple[int, int]]]:
    """
    ...
    def loop():
        ...
        while len(forests) < num_forests:
            ...
            if subset_edges not in forests and is_forest(
                net.decomposition(subset_edges)[0]
            ):
                ...
                if consecutive_fails > FAIL_LIMIT:
                    raise ValueError(
                        f"Argument 'num_forests' exceedes the number of valid forests"
                    )
    ...
    if verbose:
        with tqdm(
            total=num_forests,
            desc="Selecting a subset of forests at random",
            unit="forest",
            initial=1,
        ) as pbar:
            loop()
...
def generate_forests(
    net: Network,
    forest_generation: ForestGeneration,
    min_edges: int,
    num_forests: int,
    verbose: bool,
) -> List[List[Tuple[int, int]]]:
...
def generate_symmetric_forests(
    forest: List[Tuple[int, int]], N: int
) -> List[List[Tuple[int, int]]]:
    """
...
def scale_network(net: Network, factor: float) -> Network:
    ...
    for server in net.servers:
        ...
        for service_curve in server.service_curve:
            service_curves.append(
                RateLatency(service_curve.rate * factor, service_curve.latency)
            )
        shapers = []
    ...
    for flow in net.flows:
        ...
        for arrival_curve in flow.arrival_curve:
            arrival_curves.append(
                TokenBucket(arrival_curve.sigma * factor, arrival_curve.rho * factor)
            )
...
def __edges_to_reverse_adj_list(
...
def flow_preserving_min_depth_max_forest(
    edges: List[Tuple[int, int]], N: int, flow_path: List[int], max_depth: int = -1
) -> List[Tuple[int, int]]:
    """
